...
The workflow therefore reads the *live* pull request through `bin/fm-pr-attestation-await.sh`, which polls for up to five minutes until the attestation names the head the forge currently has, and only then hands those live facts to the gate action.
That closes the publish-order window without loosening the gate: a body that never binds is still judged against the live head, so a push that was never signed still fails with `Pipeline attestation head_sha does not match the current PR head`.
