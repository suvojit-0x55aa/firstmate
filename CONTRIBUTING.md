...
Because the attestation binds to one commit, any commit added to an already-open PR branch - including fixes the pipeline itself pushes while resolving CI findings - leaves the PR body bound to the previous head until the pipeline re-signs it.
The pipeline publishes in two steps, pushing the commit and re-signing the body moments later, and GitHub captures the `synchronize` event payload at push time, so judging that payload would fail an already-compliant PR and no later event would re-run the job.
The workflow therefore reads the *live* pull request through `bin/fm-pr-attestation-await.sh`, which polls for up to five minutes until the attestation names the commit the run was triggered for, and only then hands those live facts to the gate action.
It judges that triggering commit, never whatever the branch has drifted to while it polled, because a check result stays cached against the SHA it was recorded on: reporting the live head instead would let a branch that moves mid-wait leave a green check on a commit no attestation ever named.
That closes the publish-order window without loosening the gate: a body that never binds is still judged, and still fails with `Pipeline attestation head_sha does not match the current PR head`.
Only the pipeline's `pr` step writes the attestation, and it runs once per gate push, so a head the pipeline's CI auto-fix loop pushes afterwards is never re-signed inside that same run and no source, test, or workflow change can clear its `head_sha` mismatch - any commit written to try moves the head again.
Recover from that failure by pushing the updated branch through the gate again (`git push no-mistakes`) so the pipeline re-signs the PR body against the new head; editing the body by hand does not restore compliance, and exempting the pipeline's own pushes is not an option because that is exactly the "later push passes on an older attestation" case the check exists to catch.
