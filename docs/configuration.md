...
`bin/fm-quota-reset-arm.sh` is a concrete `when` application: it arms a per-task watch that wakes firstmate once a stuck crewmate's Claude quota resets, reading the reset time deterministically from `quota-axi --provider claude --json` (the account-wide binding window named by `quotaSemantics.effectiveAvailability[scope=all_models].limitingWindowIds`, resolved by `bin/fm-quota-reset-epoch.sh`) rather than peeking the task's pane, since quota is account-wide and never task-local. `bin/fm-quota-reset-notify.sh` is its action and stays detection-only: it re-peeks the task read-only and names the manual `bin/fm-crew-state.sh` and `bin/fm-control.sh <id> relaunch` commands, but never runs them itself. Each script's own header owns its exact flags, buffer default, and failure behavior; a quota-axi failure or timeout refuses to arm rather than guessing a reset time.
...
FM_SESSION_START_DIGEST_SOURCE_TIMEOUT=8   # seconds bounding one opted-in secondmate's charter-declared digest-source scan; a scan that exceeds it omits only that secondmate's section and says so
...
FM_RECOVERY_MARKER_ACKED_RESURFACE_SECS=3600   # minimum seconds between watcher re-arm recoveries of an acknowledged state/.watcher-down episode while the wake queue still holds rows; kept separate from FM_PAUSE_RESURFACE_SECS so each cadence tunes alone
